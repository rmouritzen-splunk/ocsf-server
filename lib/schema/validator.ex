...
defmodule Schema.Validator do
    ...
    case Schema.find_class(class_uid) do
      ...
      class ->
        if is_list(profiles) do
          validate_type(class, data, profiles, data_types)
        else
  ...
  defp validate_type(type, data, profiles, data_types) when is_map(data) do
    ...
    Enum.reduce(attributes, %{}, fn {name, attribute}, acc ->
      ...
      if attribute[:is_array] == true and value != nil and not is_list(value) do
        ...
        validate_data(acc, name, attribute, value, profiles, data_types)
      end
    end)
    |> undefined_attributes(attributes, data)
  end

  defp validate_type(type, data, _profiles, _data_type) do
  ...
  defp validate_data(acc, name, attribute, value, _profiles, data_types) when is_binary(value) do
    case attribute[:type] do
      ...
      _ ->
        validate_data_type(acc, name, attribute, value, "string_t", data_types)
    end
  end

  defp validate_data(acc, name, attribute, value, _profiles, data_types) when is_integer(value) do
    case attribute[:type] do
      "integer_t" ->
      ...
      _ ->
        validate_data_type(acc, name, attribute, value, ["integer_t", "long_t"], data_types)
    end
  end

  defp validate_data(acc, name, attribute, value, _profiles, data_types) when is_float(value) do
    validate_data_type(acc, name, attribute, value, "float_t", data_types)
  end

  defp validate_data(acc, name, attribute, value, _profiles, data_types) when is_boolean(value) do
    validate_data_type(acc, name, attribute, value, "boolean_t", data_types)
  end

  defp validate_data(acc, name, attribute, value, profiles, data_types) when is_map(value) do
    case attribute[:type] do
      "object_t" -> validate_object(acc, name, attribute, value, profiles, data_types)
      "json_t" -> acc
      type -> Map.put(acc, name, invalid_data_type(attribute, value, type))
    end
  end

  defp validate_data(acc, name, attribute, value, profiles, data_types) when is_list(value) do
    case attribute[:type] do
      "json_t" ->
        acc
      ...
      type ->
        if attribute[:is_array] do
          validate_array(acc, name, attribute, value, profiles, data_types)
        else
  ...
  defp validate_data(acc, name, attribute, nil, _profiles, _data_types) do
  ...
  defp validate_data(acc, name, _attribute, value, _profiles, _data_types) do
    Map.put(acc, name, %{
  ...
  defp validate_array(acc, _name, _attribute, [], _profiles, _data_types) do
    acc
  end

  defp validate_array(acc, name, attribute, value, profiles, data_types) do
    Logger.debug("validate array: #{name}")

    case attribute[:type] do
      "json_t" -> acc
      "object_t" -> validate_object_array(acc, name, attribute, value, profiles, data_types)
      _simple_type -> validate_simple_array(acc, name, attribute, value, profiles, data_types)
    end
  end

  defp validate_simple_array(acc, name, attribute, value, profiles, data_types) do
    {map, _count} =
  ...
  defp validate_object_array(acc, name, attribute, value, profiles, data_types) do
    case attribute[:object_type] do
      ...
      object_type ->
        ...
        {map, _count} =
          Enum.reduce(value, {Map.new(), 0}, fn data, {map, count} ->
            map = validate_type(object, data, profiles, data_types) |> add_count(map, count)
  ...
  defp validate_object(acc, name, attribute, value, profiles, data_types) do
    case attribute[:object_type] do
      "object" ->
        acc
      ...
      object_type ->
        ...
        |> validate_type(value, profiles, data_types)
        |> valid?(acc, name)
    end
  end
  ...
  defp validate_data_type(acc, name, attribute, value, value_type, data_types) do
    case attribute[:type] do
      "json_t" ->
        acc
      ...
      type ->
        if Schema.data_type?(data_types, type, value_type) do
          acc
        else
          Map.put(acc, name, invalid_data_type(attribute, value, type))
        end
